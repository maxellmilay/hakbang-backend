...
def scheduled_recalculate():
    from annotation.models import Sidewalk
...
def batch_update_accessibility_scores(sidewalks, Annotation, model_type):
    from annotation.models import Sidewalk
    ...
    for coordinates_string, _ in Sidewalk.ANCHOR_CHOICES:
    ...
    for sidewalk in sidewalks:
        data = calculate_accessibility_score(sidewalk, model, anchored_weather_data, Annotation, model_type)
        ...
        try:
            sidewalk.full_clean()
            sidewalk.save()
            print(f"Accessibility for Sidewalk {sidewalk.id} successfully calculated")
        except ValidationError as e:
            print('ERROR: ', e)

def individual_update_accessibility_scores(sidewalk, Annotation, model_type, annotation_data):
    with open('models/logistic_regression_model.pkl', 'rb') as file:
        model = pickle.load(file)
...
def calculate_accessibility_score(sidewalk, model, anchored_weather_data, Annotation, model_type, annotation_data=None):
    ...
    sidwalk_data = sidewalk.data

    flood_hazard_index = Annotation.FLOOD_HAZARD[str(sidwalk_data['hazard'])]
    weather = anchored_weather_data[sidewalk.anchor]

    total_key_areas = sum(sidwalk_data['nearPlacesFrequency'].values())
    simplified_population = sidwalk_data['population']/1000

    zoning_area_index = Annotation.ZONING_AREA[sidwalk_data['zone']]
